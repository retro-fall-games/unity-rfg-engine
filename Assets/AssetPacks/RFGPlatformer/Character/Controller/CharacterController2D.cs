...
namespace RFG
{
  ...
  public class CharacterController2D : MonoBehaviour, IMoveable
  {
    ...
    private void Move(Vector2 deltaMovement)
    {
      ...
      deltaMovement.x = Mathf.Min(deltaMovement.x, Parameters.maxVelocity.x);
      deltaMovement.y = Mathf.Min(deltaMovement.y, Parameters.maxVelocity.y);
    ...
    private void CorrectHorizontalPlacement(ref Vector2 deltaMovement, bool isRight)
    {
      ...
      for (var i = 1; i < numberOfHorizontalRays - 1; i++)
      {
        ...
        _raycastHit = RFG.Physics2D.Raycast(rayVector, rayDirection, halfWidth, movingPlatformMask, Color.yellow);
        ...
        offset = isRight ? ((_raycastHit.point.x - _transform.position.x) - halfWidth) : (halfWidth - (_transform.position.x - _raycastHit.point.x));
      }
    ...
    private void MoveHorizontally(ref Vector2 deltaMovement)
    {
      ...
      for (var i = 0; i < numberOfHorizontalRays; i++)
      {
        ...
        {
          _raycastHit = RFG.Physics2D.Raycast(rayVector, rayDirection, rayDistance, platformMask, Color.red);
        }
        else
        {
          _raycastHit = RFG.Physics2D.Raycast(rayVector, rayDirection, rayDistance, platformMask & ~oneWayPlatformMask, Color.green);
        }
        ...
        if (i == 0 && HandleHorizontalSlope(ref deltaMovement, Vector2.Angle(_raycastHit.normal, Vector2.up), isGoingRight))
        {
          ...
          {
            float flushDistance = Mathf.Sign(deltaMovement.x) * (_raycastHit.distance - Parameters.skinWidth);
    ...
    private void MoveVertically(ref Vector2 deltaMovement)
    {
      ...
      for (var i = 0; i < numberOfVerticalRays; i++)
      {
        ...
        var _raycastHit = RFG.Physics2D.Raycast(rayVector, rayDirection, rayDistance, mask, Color.blue);
        ...
        if (!isGoingUp)
        {
          var verticalDistanceToHit = _transform.position.y - _raycastHit.point.y;
          if (verticalDistanceToHit < standingOnDistance)
          {
            standingOnDistance = verticalDistanceToHit;
            StandingOn = _raycastHit.collider.gameObject;
          }
        }

        // Determine whats the furtherest we can move down without hitting anything
        deltaMovement.y = _raycastHit.point.y - rayVector.y;
    ...
    private bool HandleHorizontalSlope(ref Vector2 deltaMovement, float angle, bool isGoingRight)
    {
      ...
      if (angle < Parameters.maxSlopeAngle)
      {
        if (deltaMovement.y < 0.07f)
        {
          ...
          _raycastHit = RFG.Physics2D.Raycast(rayOrigin, deltaMovement.normalized, deltaMovement.magnitude, stairsMask, Color.magenta);
    ...
    private void HandleVerticalSlope(ref Vector2 deltaMovement)
    {
      ...
      _raycastHit = RFG.Physics2D.Raycast(slopeRayVector, direction, slopeDistance, stairsMask, Color.red);
      ...
      {
        var slopeModifier = slopeSpeedMultiplier.Evaluate(-angle);
        deltaMovement.y += _raycastHit.point.y - slopeRayVector.y - Parameters.skinWidth;
        // deltaMovement.x *= slopeModifier;
