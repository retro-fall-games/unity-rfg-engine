...
namespace RFG.Core
{
  ...
  public class Aim
  {
    ...
    public Vector2 GetCurrentAim()
    {
      ...
      {
        currentAngle = RFG.Core.Math.RoundToClosest(currentAngle, _possibleAngleValues);
      }

      currentAngle = Mathf.Clamp(currentAngle, minAngle, maxAngle);

      _currentAim = _currentAim.magnitude == 0f ? Vector2.zero : RFG.Core.Math.RotateVector2(Vector2.right, currentAngle);
