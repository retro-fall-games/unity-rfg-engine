  ...
  public class CharacterController2D : MonoBehaviour, IMoveable
  {
    ...
    protected void CastRaysToTheSides(float raysDirection)
    {
      ...
      for (int i = 0; i < Parameters.NumberOfHorizontalRays; i++)
      {
        ...
        {
          _sideHitsStorage[i] = RFG.Core.Physics2D.RayCast(rayOriginPoint, raysDirection * Vector2.right, horizontalRayLength, PlatformMask, Color.red, true);
        }
        else
        {
          _sideHitsStorage[i] = RFG.Core.Physics2D.RayCast(rayOriginPoint, raysDirection * Vector2.right, horizontalRayLength, PlatformMask & ~OneWayPlatformMask & ~OneWayMovingPlatformMask, Color.red, true);
        }
    ...
    private void CastRaysBelow()
    {
      ...
      for (int i = 0; i < Parameters.NumberOfVerticalRays; i++)
      {
        ...
        {
          _belowHitsStorage[i] = RFG.Core.Physics2D.RayCast(rayOriginPoint, -transform.up, rayLength, _raysBelowLayerMaskPlatformsWithoutOneWay, Color.magenta, true);
        }
        else
        {
          _belowHitsStorage[i] = RFG.Core.Physics2D.RayCast(rayOriginPoint, -transform.up, rayLength, _raysBelowLayerMaskPlatforms, Color.magenta, true);
        }
    ...
    private void CastRaysAbove()
    {
      ...
      for (int i = 0; i < Parameters.NumberOfVerticalRays; i++)
      {
        ...
        _aboveHitsStorage[i] = RFG.Core.Physics2D.RayCast(rayOriginPoint, (transform.up), rayLength, PlatformMask & ~OneWayPlatformMask & ~OneWayMovingPlatformMask, Color.green, true);
    ...
    private void StickToSlope()
    {
      ...
      _stickRaycastLeft = RFG.Core.Physics2D.RayCast(_rayCastOrigin, -transform.up, rayLength, _raysBelowLayerMaskPlatforms, Color.green, true);

      _rayCastOrigin.x = _boundsBottomRightCorner.x;
      _rayCastOrigin.x += _newPosition.x;
      _stickRaycastRight = RFG.Core.Physics2D.RayCast(_rayCastOrigin, -transform.up, rayLength, _raysBelowLayerMaskPlatforms, Color.green, true);
      ...
      if (belowSlopeAngleLeft > 0f && belowSlopeAngleRight < 0f)
      {
        _stickRaycast = RFG.Core.Physics2D.BoxCast(_boundsCenter, Bounds, Vector2.Angle(transform.up, Vector2.up), -transform.up, rayLength, _raysBelowLayerMaskPlatforms, Color.yellow, true);
        if (_stickRaycast)
        {
    ...
    private void MoveTransform()
    {
      ...
      {
        _stickRaycast = RFG.Core.Physics2D.BoxCast(_boundsCenter, Bounds, Vector2.Angle(transform.up, Vector2.up), _newPosition.normalized, _newPosition.magnitude, PlatformMask, Color.red, true);
        if (_stickRaycast)
        {
    ...
    private void ComputeDistanceToTheGround()
    {
      ...
      _distanceToTheGroundRaycast = RFG.Core.Physics2D.RayCast(_rayCastOrigin, -transform.up, Parameters.DistanceToTheGroundRayMaximumLength, _raysBelowLayerMaskPlatforms, Color.cyan, true);
    ...
    public bool CanGoBackToOriginalSize()
    {
      ...
      bool headCheckLeft = RFG.Core.Physics2D.RayCast(_originalSizeRaycastOrigin, transform.up, headCheckDistance, PlatformMask - OneWayPlatformMask, Color.gray, true);

      _originalSizeRaycastOrigin = _boundsTopRightCorner + (Vector2)transform.up * _smallValue;
      bool headCheckRight = RFG.Core.Physics2D.RayCast(_originalSizeRaycastOrigin, transform.up, headCheckDistance, PlatformMask - OneWayPlatformMask, Color.gray, true);
